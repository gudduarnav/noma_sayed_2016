...
% ---- Plot the figure ----
f = figure('Name', 'Figure 9', 'NumberTitle', 'off');

plot(list_SNR, list_sum_rate_ratio, 'k-', 'linewidth', 2)
hold on
u1_ratio = list_rate_ratio(1, :);
u2_ratio = list_rate_ratio(2, :);
u3_ratio = list_rate_ratio(3, :);
u4_ratio = list_rate_ratio(4, :);

plot(list_SNR, u1_ratio, 'b--', 'LineWidth', 2);
plot(list_SNR, u2_ratio, 'r--', 'LineWidth', 2);
plot(list_SNR, u3_ratio, 'g--', 'LineWidth', 2);
plot(list_SNR, u4_ratio, 'k--', 'LineWidth', 2);


hold off
legend('Total Rate', 'User 1', 'User 2', 'User 3', 'User 4', 'Location', 'NorthWest');
grid on
xlabel('SNR (dB)')
